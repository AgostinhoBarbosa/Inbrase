<?php
    ...
    class SystemDocumentForm extends TPage
    {
        ...
        public function __construct( $param )
        {
            ...
            $campo_title       = [new TLabel( 'Título' ), $title, $id];
            $campo_description = [new TLabel( 'Descrição' ), $description];
            $campo_category    = [new TLabel( 'Categoria' ), $category_id];
            $campo_submission  = [new TLabel( 'Data Submissão' ), $submission_date];
            $campo_archive     = [new TLabel( 'Data Arquivamento' ), $archive_date];

            $this->form->addFields( $campo_title );
            $this->form->addFields( $campo_description );
            $this->form->addFields( $campo_category );
            $this->form->addFields( $campo_submission, $campo_archive )->layout = ['col-sm-2', ' col-sm-2'];
        ...
        public static function onFechaRightPanel( $param )
        {
        ...
        public function onSave( $param )
        {
            try {
                ...
                $object->filename       = TSession::getValue( 'SystemDocumentUpload_file' );
                $object->clearParts();
                ...
                $source_file = 'tmp/' . TSession::getValue( 'SystemDocumentUpload_file' );
            ...
            try {
                if ( isset( $param[ 'key' ] ) ) {
                    ...
                    if ( empty( $param[ 'hasfile' ] ) ) {
                        TSession::setValue( 'SystemDocumentUpload_file', $object->filename );
                    }
                } else {
                    $this->form->clear();
                }
